 ...
 */
  $ethnosimosrc = 'http://sde-agrin.ait.sch.gr/sde/websvc/images/ethnosimo.jpg';
...
if (isset($_GET['tag']) && $_GET['tag'] != '')
{
    ...
    { 
		...
		l.asemester as asem, l.bsemester as bsem, class.ClassName as classname, class.ClassID as classid,
		lessons.LessonName As lessonname, l.*, g.amathisiakiporeia, g.bmathisiakiporeia, g.asynergasia, g.bsynergasia, g.bendiaferon as bendiaferongeneral, g.aendiaferon  as aendiaferongeneral, g.adesmefsi, g.bdesmefsi
		...
		LEFT JOIN `_students_general` AS g ON g.StudentID=l.StudentID
		
		
		WHERE l.eduyear='".$eduyear."'
		AND `_students_class`.eduperiod='".$eduyear."'
		AND g.eduyear='".$eduyear."'
		
		AND l.StudentID IN
				(SELECT students.StudentID from students INNER JOIN `_students_class` ON `_students_class`.StudentID = students.StudentID
					WHERE students.IsActive=1
					AND `_students_class`.eduperiod='".$eduyear."'
					AND `_students_class`.ClassID IN
					(SELECT ClassID FROM `_class_teachers` WHERE TeacherID = ".$teacherid." AND eduyear='".$eduyear."'))
		
		
		AND l.LessonID IN (SELECT LessonID from `_lessons_teachers` WHERE TeacherID = ".$teacherid.")
		ORDER BY lessonname, classname, lname, fname;";
	    ...
	    if ($result)
	    {
			...
			if (isset($_GET['type']) && ($_GET['type']=='w')) //check for $w again
			{
				...
				//$content = "This is a test page";
				 ...
				 $fpagecont .= '<img src="'.$ethnosimosrc.'" border=0 height=60px/><br />ΕΛΛΗΝΙΚΗ ΔΗΜΟΚΡΑΤΙΑ<br />ΥΠΟΥΡΓΕΙΟ ΠΟΛΙΤΙΣΜΟΥ, ΠΑΙΔΕΙΑΣ ΚΑΙ ΘΡΗΣΚΕΥΜΑΤΩΝ<br />ΓΕΝΙΚΗ ΓΡΑΜΜΑΤΕΙΑ ΔΙΑ ΒΙΟΥ ΜΑΘΗΣΗΣ ΚΑΙ ΝΕΑΣ ΓΕΝΙΑΣ<br />ΔΙΕΥΘΥΝΣΗ ΔΙΑ ΒΙΟΥ ΜΑΘΗΣΗΣ</div><br /><br /><br /><br /><br /><br /><br /><br />';
				...
				$studentcount=0;
			        ...
			        while($row = mysql_fetch_assoc($result))
				 {
				    ...
				    $lpagecont .='<span style="font-weight:bold; font-size:1em;">Μαθησιακή Πορεία</span><br /><br /><br />'.$row[$mathisiakiporeia].'<br /><br /><br />';
				    $lpagecont .='<span style="font-weight:bold; font-size:1em;">Συνεργασία/ Συμμετοχή σε Ομαδικές Δραστηριότητες<br />(Εργαστήρια-Σχέδια Δράσης)</span><br /><br />'.$row[$synergasia].'<br /><br /><br /><br /><br /><br />';
				    $lpagecont .='<span style="font-weight:bold; font-size:1em;">Ενδιαφέρον-Ανάληψη Πρωτοβουλιών</span><br /><br />'.$row[$endiaf].'<br /><br /><br /><br />';
				    $lpagecont .='<span style="font-weight:bold; font-size:1em;">Δέσμευση-Αποτελεσματικότητα</span><br /><br /><br />'.$row[$desmefsi].'<br /><br /><br />';
				    $lpagecont .='<table width="100%" cellspacing="0" style="border: none;"><tr>';
				    $lpagecont .='<td width=50% style="border: none;font-weight:bold;vertical-align: top; text-align:center;" >Ο ΔΙΕΥΘΥΝΤΗΣ<br /><br /><br /><br /><br />ΝΑΣΙΟΣ ΝΙΚΟΛΑΟΣ</td><td width=50% style="border: none;font-weight:bold;vertical-align: top; text-align:center;">Ο ΥΠΟΔΙΕΥΘΥΝΤΗΣ<br /><br /><br /><br /><br />ΑΘΑΝΙΤΗΣ ΑΝΔΡΕΑΣ</td></tr></table>';
...
INNER JOIN class ON class.ClassID = `_students_class`.ClassID
	    INNER JOIN lessons ON lessons.LessonID = l.LessonID WHERE l.eduyear =  '".$eduyear."' AND class.classID>3
	    ...
	    if ($result)
	    {
			...
			if (isset($_GET['type']) && ($_GET['type']=='w')) //check for $w again
			{
				...
				//$content = "This is a test page";
				 ...
				 $fpagecont = '<div style="font-weight:bold; text-align:left; margin-bottom:60px;">';
				 $fpagecont .= '<img src="'.$ethnosimosrc.'" border=0 height=60px/><br />ΕΛΛΗΝΙΚΗ ΔΗΜΟΚΡΑΤΙΑ<br />ΥΠΟΥΡΓΕΙΟ ΠΑΙΔΕΙΑΣ ΚΑΙ ΘΡΗΣΚΕΥΜΑΤΩΝ<br />ΓΕΝΙΚΗ ΓΡΑΜΜΑΤΕΙΑ ΔΙΑ ΒΙΟΥ ΜΑΘΗΣΗΣ<br />ΙΔΡΥΜΑ ΝΕΟΛΑΙΑΣ ΚΑΙ ΔΙΑ ΒΙΟΥ ΜΑΘΗΣΗΣ</div><br /><br /><br /><br /><br /><br /><br /><br />';
