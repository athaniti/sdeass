...
var getStudentGrades = function(studentid, teacherid){
	...
	$.getJSON(websvcroot+'services.php?tag=grade&eduyear=1213&studentid='+studentid+'&teacherid='+teacherid+'&callback=?',
	    function(data) {
        	...
        	if (data.success == '1') {
				//return data.userrole;
				$("#atetramino").html(data.studentgrades[0].asemester);
				$("#btetramino").html(data.studentgrades[0].bsemester);
...
var getStudentData = function(studentid) {
	$.getJSON(websvcroot+'services.php?tag=studdata&studentid='+studentid+'&callback=?', function(data) {
        	if (data.success == '1') {
			$("#gradestitle").html(data.studentfname+' '+data.studentlname);
		}
			return '';
	});
