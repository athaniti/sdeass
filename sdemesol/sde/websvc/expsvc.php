 ...
 */
  ...
  $titletext = 'ΕΛΛΗΝΙΚΗ ΔΗΜΟΚΡΑΤΙΑ<br />ΥΠΟΥΡΓΕΙΟ ΠΑΙΔΕΙΑΣ ΘΡΗΣΚΕΥΜΑΤΩΝ ΚΑΙ ΑΘΛΗΤΙΣΜΟΥ<br />ΓΕΝΙΚΗ ΓΡΑΜΜΑΤΕΙΑ ΕΠΑΓΓΕΛΜΑΤΙΚΗΣ ΕΚΠΑΙΔΕΥΣΗΣ,<br />ΚΑΤΑΡΤΙΣΗΣ ΚΑΙ ΔΙΑ ΒΙΟΥ ΜΑΘΗΣΗΣ<br />ΔΙΕΥΘΥΝΣΗ ΔΙΑ ΒΙΟΥ ΜΑΘΗΣΗΣ';
  ...
  function mb_lower_safe($v, string $fallbackEnc = 'UTF-8'): string {
    $s = (string)($v ?? '');
    if ($s === '') return '';
    $enc = mb_detect_encoding($s, mb_detect_order(), true) ?: $fallbackEnc;
...
if (isset($_GET['tag']) && $_GET['tag'] != '')
{
    ...
    $response["whereami"]=$qry;
		...
		if ($result) {
			...
			// headers για ΚΑΙ Word και Excel (download)
			...
			if (isset($_GET['type']) && ($_GET['type']=='w')) //check for $w again
			{
				...
				$contentstart = "<strong>Σ.Δ.Ε. Μεσολογγίου</strong><br />Αξιολόγηση του εκπαιδευτικού: <strong>".$user['UserLname']." ".$user['UserFname']."</strong> για το ".$semestname." Τετράμηνο του Σχολικού Έτους ".$eduyearstr."<br /><br />";
				$classname = '';
				$lessonname = '';
				$newclass = '';
				$newlesson = '';
				...
				if (ob_get_length()) { ob_end_clean(); }   // καθάρισε buffer πριν το download
				header('Content-Type: application/'.$file_type.'; charset=utf-8');
    ...
    elseif ($tag== 'perproject')
    {
		...
		if ($result) {
			...
			if (isset($_GET['type']) && ($_GET['type']=='w')) //check for $w again
			{
				...
				// καθάρισε τυχόν output buffer (αν υπάρχει)
			...
			if (isset($_GET['type']) && ($_GET['type']=='w')) //check for $w again
			{
				...
				$lpagecont='';
			        while($row = $result->fetch_assoc())
				 {
				    ...
				    $lpagecont .='<span style="font-weight:bold; font-size:1em;">Μαθησιακή Πορεία</span><br /><br /><br />'.$poreiastart.mb_lower_safe($row[$mathisiakiporeia] ?? null).$poreiaend.'<br /><br /><br />';
					$lpagecont .='<span style="font-weight:bold; font-size:1em;">Συνεργασία/ Συμμετοχή σε Ομαδικές Δραστηριότητες<br />(Εργαστήρια-Σχέδια Δράσης)</span><br /><br />'.$synergasiastart.mb_lower_safe($row[$synergasia] ?? null).$synergasiaend.'<br /><br /><br /><br /><br /><br />';
					$lpagecont .='<span style="font-weight:bold; font-size:1em;">Ενδιαφέρον-Ανάληψη Πρωτοβουλιών</span><br /><br />'.$endiaferonstart.mb_lower_safe($row[$endiaf] ?? null).$endiaferonend.'<br /><br /><br /><br />';
					$lpagecont .='<span style="font-weight:bold; font-size:1em;">Δέσμευση-Αποτελεσματικότητα</span><br /><br /><br />'.$desmefsistart.mb_lower_safe($row[$desmefsi] ?? null).$desmefsiend.'<br /><br /><br />';
	    ...
	    if ($result)
	    {
			...
			if (isset($_GET['type']) && ($_GET['type']=='w')) //check for $w again
			{
				...
				//	 echo("$title\n\n");
				 ...
				 $fpagecont .= '<div style="font-weight:bold; text-align:center">ΣΧΟΛΕΙΟ ΔΕΥΤΕΡΗΣ ΕΥΚΑΙΡΙΑΣ ΜΕΣΟΛΟΓΓΙΟΥ<br /></div>';
				 $fpagecontend = '<br /><br /><div style="font-weight:bold; text-align:right;  margin-top:40px;">ΣΧΟΛΙΚΟ ΕΤΟΣ '.$eduyearstr.'</div>'.$pagesep;
        ...
        $lpagecont='';
				...
				$count = 0;
			        while($row = $result->fetch_assoc())
				 {
					...
					$average .= '<td style="border: 1px solid #000; padding:3px;">('.$total.') - '.strval($ints).' και '.strval($decs).' όγδοα.<br />'.$roundedresult.'</td>';
					$average .= '</tr>';
				 ...
				 $average = '<tr style="border: 1px solid #000;">';
				 $average .= '<td style="border: 1px solid #000; padding:3px;">Μέσος όρος</td>';
				 $average .= '<td style="border: 1px solid #000; padding:3px;">'.strval($ints).' / '.strval($decs).' όγδοα.';
				 $average .= '</tr>';
				 //$content = $content.$extracontend;
				 $content .= $extracontend . $lpagecont;
